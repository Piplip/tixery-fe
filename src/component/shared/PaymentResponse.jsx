...
function PaymentResponse(){
    ...
    useEffect(() => {
        if(!processed.current){
            ...
            const url = `/payment/${initialType === 'success' ? 'stripe/success' : 'stripe/failure'}?order-id=${new URLSearchParams(location.search).get('orderID')}`
        ...
        <Stack className={isLoading ? '' : initialType && response?.status === 'success' ? 'payment-response-success' : 'payment-response-failed'} justifyContent={'center'} alignItems={'center'} height={'100dvh'}>
            {isLoading ?
                <Stack rowGap={5} alignItems={'center'}>
                    <Typography variant={'h4'}>Processing your payment order. Please wait...</Typography>
                ...
                initialType && response?.status === 'success' ?
                    <Box className="payment-success">
                        ...
                        <Typography variant="h5" className="payment-success__title" fontFamily={'Roboto Slab'} fontWeight={500}>
                            {response?.message || (initialType ? "Ticket Payment Successful" : "Ticket Payment Failed")}
                        </Typography>
                        <Typography variant="body1" className="payment-success__transaction">
                            Transaction ID: {response.sessionID}
                        </Typography>

                        <Box className="payment-success__details">
                            <Typography variant="h6">
                                Paid amount: <span className="payment-success__amount">{formatCurrency(response.amount / 100, response.currency)}</span>
                            </Typography>
                            <Typography variant="h6">
                                Paid by <span className="payment-success__method">{getUserData('fullName')}</span>
                            </Typography>
                        </Box>
                        <Link to={`/u/${getUserData('profileID')}`}>
                            <Button variant={'contained'} color={'primary'} sx={{marginTop: 3}}>Take me to my tickets</Button>
                        </Link>
                    </Box>
                    :
                    <Box className="payment-failed">
                        ...
                        <Box className="payment-failed__details">
                            <Typography variant="h6">
                                Pay with <span className="payment-failed__method">{response.paymentMethod}</span>
                            </Typography>
                        </Box>
                        <Link to={`/events/${response.eventID}`}>
                            <Button variant={'contained'} color={'error'} sx={{marginTop: 3}}>Retry</Button>
                        </Link>
                    </Box>
            }
        </Stack>
